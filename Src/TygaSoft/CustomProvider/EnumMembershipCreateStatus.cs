...
namespace TygaSoft.CustomProvider
{
    public class EnumMembershipCreateStatus
    {
        public EnumMembershipCreateStatus(MembershipUser user)
        {
            ...
            else if (user.IsLockedOut) throw new ArgumentException(MC.L_AccountLock);
            else if (!user.IsApproved) throw new ArgumentException(MC.L_AccountAllow);
            else throw new ArgumentException(MC.L_InvalidPsw);
        }
